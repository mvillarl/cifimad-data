...
class AttendeeQuery extends \yii\db\ActiveQuery
{
    public function __construct( $modelClass, array $config = null) {
        ...
        $this->joinWith('idEvent0')->joinWith('idMember0')->joinWith('idSource0')
            ...
            ->select('cif_attendees.*, cif_events.name eventName, cif_members.badgeName, cif_members.badgeSurname, cif_members.name, cif_members.surname, cif_sources.name sourceName, cif_members.nif'
    ...
    public function andFilterSearchMember ($term) {
        $term = trim($term);
        if (strlen ($term)) {
            $this->andWhere(['or', 'cif_members.badgeName LIKE :term', 'cif_members.badgeSurname LIKE :term'], [':term' => '%'.$term.'%']);
        }
        return $this;
    }
